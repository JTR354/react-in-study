  ...
  switch (action.type) {
    ...
    case "TOGGLE":
      return list.map((item) => ({
        ...item,
        undo: item.id === action.id ? !item.undo : item.undo,
      }));
    default:
      return list;
  }
...
const store = createStore(
  rootReducer,
  initalState,
  enhancerCompose(applyMiddleware())
);
...
const Panel = connect()((props) => {
  ...
  return (
    <>
      <h4>{title}</h4>
      <ul>{props.list.map((item) => props.render({ key: item.id, item }))}</ul>
    </>
  );
});
  ...
  return (
    <>
      ...
      <Panel
        title="未完成"
        list={props.undoList}
        render={(args) => <ToDoItem {...args}></ToDoItem>}
      ></Panel>
      <Panel
        title="已完成"
        list={props.doList}
